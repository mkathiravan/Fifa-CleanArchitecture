...
package com.example.kathir.fifa_cleanarchitecture.data.local;

import android.content.Context;
import android.support.annotation.NonNull;

import com.example.kathir.fifa_cleanarchitecture.data.entity.TeamEntity;
import com.example.kathir.fifa_cleanarchitecture.data.repository.datasource.mapper.TeamEntityJsonMapper;
...
public class LocalImpl implements LocalApi {
  ...
  @Override public Observable<List<TeamEntity>> teamEntityList() {
    return Observable.create(emitter -> {
      ...
      if (teamEntityList != null) {
        ...
        emitter.onError(
            new Throwable("Error getting team data list from the local json (fifa_data.json)"));
      }
    });
  }
  ...
  @Override public Observable<TeamEntity> teamEntity(final String flag) {
    return Observable.create(emitter -> {
      ...
      if (teamEntity != null) {
        ...
        emitter.onError(
            new Throwable("Error getting team data by flag from the local json (fifa_data.json)"));
      }
    });
  }
  ...
  private String getResponseFromLocalJson() {
    final String EURO_DATA_FILE = "fifa_data.json";
    String str = "";
    try {
